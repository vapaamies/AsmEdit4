...
begin
  ...
  if STLinkConfForm.ConnectTypeCheckBox.Checked then
    begin
      if STLinkConfForm.SWDRadioButton.Checked then str:='SWD '
        else str:='JTAG '
    end;
  ...
  MainForm.MessagesListBox.Items.Insert(0,'Параметры запуска');
  MainForm.MessagesListBox.Items.Insert(1,'Утилита программатора: '+STLinkConfForm.STLinkEdit.Text);
  MainForm.MessagesListBox.Items.Insert(2,'Способ подключения: '+str);
  MainForm.MessagesListBox.Items.Insert(3,'Файл прошивки: '+filename);
  MainForm.MessagesListBox.Items.Insert(4,'Адрес: '+Project.Project_MCU_TargetAdr);
  MainForm.MessagesListBox.Items.Insert(5,'');
end;
